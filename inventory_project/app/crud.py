...
def create_product(db: Session, product_data: ProductCreate) -> Product:
    ...
    logger.info(
        f"Created product: {db_product.id} - {db_product.name}"
    )
...
def update_product(
    ...
    logger.info(
        f"Updated product: {db_product.id} - {db_product.name}"
    )
...
def update_product_stock(
    ...
    if new_stock < 0:
        raise ValueError(
            f"Not enough stock. Current: {db_product.stock}, "
    ...
    logger.info(
        f"Updated stock for product {product_id}: {new_stock}"
    )
